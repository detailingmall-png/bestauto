#!/usr/bin/env python3
"""
Update EN blog articles in Tilda HTML export files with new content from markdown.
...
# Paths
# ──────────────────────────────────────────────
ARTICLES_DIR = Path("/Users/fedorzubrickij/Downloads/seo blog/en/all_articles")
...
# EN pages use direct slugs — no overrides needed
SLUG_OVERRIDES = {}
...
def parse_md_file(filepath: Path) -> dict:
    ...
    url_match = re.search(r"bestauto\.ge/en/blog/([^\s/]+)", content)
...
def main():
    ...
    for pid, data in page_map.items():
        if data.get("lang") == "en" and data.get("slug", "").startswith("blog/"):
