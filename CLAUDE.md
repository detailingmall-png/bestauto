...
### Грамматика anchor'а (ОБЯЗАТЕЛЬНО)

Инжектор заменяет `originalPhrase` на `anchor` дословно. Если `anchor` в именительном, а `originalPhrase` в косвенном падеже — текст станет грамматически неправильным.

**Алгоритм выбора anchor-позиции:**

1. **Лучший вариант — wrap nominative:** найти в тексте статьи место, где HF-ключевик стоит дословно в именительном падеже → `originalPhrase == anchor`. Текст не меняется, только оборачивается в ссылку.

2. **Нет именительного — declined anchor:** использовать падежную форму, которая уже стоит в тексте → `originalPhrase == anchor` (declined). Добавить эту форму в `seo-service-keywords.ts` с комментарием `// inflected — valid anchor`. Прецедент: `'керамическим покрытием'` уже есть в словаре.

3. **Keyword вообще не вписывается** → не добавлять ссылку на эту статью. Лучше не линковать, чем вставлять натянуто.

**НИКОГДА** не делать: `originalPhrase` в косвенном падеже + `anchor` в именительном → результат "Для нанесение керамики", "в сторону защитная пленка" и т.п.
