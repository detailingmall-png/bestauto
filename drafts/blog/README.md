...
**2. Pexels API (основной источник для hero-фото)** — бесплатная коммерческая лицензия, реальные фото, выглядят профессионально.
- Плюсы: бесплатно, быстро, реальные фотографии (не AI-артефакты).
- Минусы: не эксклюзив — то же фото теоретически может стоять на другом сайте.
- **Когда использовать**: default для hero-фото каждой статьи. Поиск по cluster-specific запросу (см. `PUBLICATION-AGENT-BRIEF.md`), показать user топ-5 результатов, дождаться выбора, скачать, сконвертировать в WebP.
- **API key**: `PEXELS_API_KEY` в `~/.zshrc`.

**3. AI-генерация через fal.ai / Flux / SDXL** — fallback, когда Pexels не даёт подходящего кадра или для декоративных inline-шотов.
...
- **Когда использовать**: для декоративных inline-шотов, абстрактных концепт-визуалов (физика SiO₂, схема слоёв PPF), когда в архиве и на Pexels пусто.
