...
.ba-blog-cta__title{font-size:30px;color:var(--ba-color-accent) !important;font-weight:700 !important;margin:0 0 16px !important;font-family:var(--ba-font-family);line-height:1.3}
.ba-blog-cta__descr{font-size:18px;line-height:1.5;color:var(--ba-color-text-muted) !important;margin:0 0 32px;font-family:var(--ba-font-family)}
.ba-blog-cta__btn{display:inline-block;font-size:16px;background:var(--ba-color-accent);color:var(--ba-color-bg) !important;font-weight:600;padding:14px 40px;border-radius:20px;text-decoration:none !important;font-family:var(--ba-font-family);transition:background var(--ba-duration-fast) var(--ba-ease-default);box-shadow:var(--ba-shadow-medium)}
.ba-blog-cta__btn:hover{background:var(--ba-color-accent-hover)}
.ba-blog-cta__btn:focus-visible{outline:2px solid #fff;outline-offset:2px}
...
export function generateBlogCtaHtml(lang: string, baseSlug: string): string {
  ...
  return `<div id="ba-blog-cta" style="position:relative;left:50%;transform:translateX(-50%);width:100vw;padding:48px 24px;background:var(--ba-color-bg);box-sizing:border-box;margin:0;">
