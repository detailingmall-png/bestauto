...
export const POPUP_SHIM = `<style>.t-popup .t-slds__item{width:100%!important;float:none!important}.t-popup .t-slds__item:not(.t-slds__item_active){display:none}.t746__imgwrapper{position:relative;overflow:hidden}.t746__separator{padding-bottom:65.12%}.t-popup .t-slds__bgimg.t-bgimg--real{background:none!important}</style>
<script>
function t_popup__showPopup(t){if(!t)return;t.querySelectorAll('.t-slds__bgimg').forEach(function(bg){if(bg.querySelector('img'))return;var src=bg.getAttribute('data-original');if(!src){var m=(bg.getAttribute('style')||'').match(/url\\(['"]?([^'"\\)\\s]+)/);if(m)src=m[1]}if(!src)return;var sep=bg.parentElement&&bg.parentElement.querySelector('.t746__separator');var img=document.createElement('img');img.style.cssText='width:100%;height:100%;object-fit:contain;display:block;background:#000';img.alt=bg.getAttribute('aria-label')||'';img.onload=function(){requestAnimationFrame(function(){if(sep&&img.naturalWidth&&img.naturalHeight)sep.style.paddingBottom=(img.naturalHeight/img.naturalWidth*100)+'%'})};img.src=src;bg.classList.add('t-bgimg--real');bg.appendChild(img)});t.style.display='block';var _done=false;var _show=function(){if(_done)return;_done=true;t.focus();var c=t.querySelector('.t-popup__container');if(c)c.classList.add('t-popup__container-animated');t.classList.add('t-popup_show');document.body.classList.add('t-body_popupshowed');t_popup__trapFocus(t)};setTimeout(_show,80);if(typeof requestAnimationFrame!=='undefined'){requestAnimationFrame(function(){requestAnimationFrame(_show)})}}
